    ...
    [UIView animateKeyframesWithDuration:1 delay:delay options:UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
        ...
        if (ws.currToyIsGold && !ws.isAnimStop) {
            ...
            [self.coinImageView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.width.height.equalTo(ws.toyView.mas_height).multipliedBy(0.15);
                make.left.equalTo(ws.toyView.mas_right).multipliedBy(0.2744);
                make.bottom.equalTo(ws.toyView.mas_bottom).multipliedBy(1 - 0.20625);
            }];
